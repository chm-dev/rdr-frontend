...
const { NetworkFirst, StaleWhileRevalidate, NetworkOnly, CacheFirst } = workbox.strategies;
const { ExpirationPlugin } = workbox.expiration;
const { precacheAndRoute } = workbox.precaching;
const { googleFontsCache, imageCache, offlineFallback, staticResourceCache } = workbox.recipes;
const cacheName = cacheNames.runtime;

const backendURL = 'https://rdr.chm.codes:1337'
...
registerRoute( ( { url } ) => url.href.includes(backendURL) && url.pathname !== '/add', new StaleWhileRevalidate( { cacheName: 'articles' } ) );
